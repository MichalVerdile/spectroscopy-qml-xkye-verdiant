...
def main(analytical_data, base_out_path, seed, n_folds, use_kfold):
    # All columns to process
    columns_to_process = ["h_nmr_spectra", "c_nmr_spectra", "ir_spectra", "pos_msms", "neg_msms"]

    # Map column names to actual parquet column names
    column_mapping = {
        "h_nmr_spectra": "h_nmr_spectra",
        "c_nmr_spectra": "c_nmr_spectra",
        "ir_spectra": "ir_spectra",
        "pos_msms": "msms_positive_40ev",
        "neg_msms": "msms_negative_40ev",
    }

    print(f"\n{'='*60}")
    print(f"Loading data for all columns")
    print(f"{'='*60}")

    # Load all columns at once
    columns_to_load = ["smiles"] + list(column_mapping.values())
    ...
    for i, parquet_file in enumerate(parquet_files):
        ...
        # Process MSMS columns
        data["msms_positive_40ev"] = [make_msms_spectrum(s) for s in data["msms_positive_40ev"]]
        data["msms_negative_40ev"] = [make_msms_spectrum(s) for s in data["msms_negative_40ev"]]
    ...
    print(f"\n{'='*60}")
    print(f"Training multimodal model with all columns")
    print(f"{'='*60}")

    # Concatenate all spectra columns into single input
    X_data = np.concatenate([
        np.stack(training_data[column_mapping[col]].to_list())
        for col in columns_to_process
    ], axis=1)
    ...
    if use_kfold:
        ...
        print(f"\n{'='*60}")
        print(f"Cross-Validation Results (Multimodal):")
        ...
        print(f"\n{'='*60}")
        print(f"FINAL TEST RESULTS (Multimodal):")
        print(f"Test F1 Score: {test_f1:.4f}")
        print(f"{'='*60}")

        # Save results
        out_path = base_out_path / "multimodal" / "k_fold"
        ...
        # Save the best model
        best_model.save(str(out_path / "multimodal_model.keras"))
        ...
        # Save results
        out_path = base_out_path / "multimodal" / "original"
        out_path.mkdir(parents=True, exist_ok=True)
        ...
        # Save model
        model.save(str(out_path / "multimodal_model.keras"))
