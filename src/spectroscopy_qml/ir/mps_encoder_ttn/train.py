...
from spectroscopy_qml.ir.mps_encoder_ttn.config import (
    DATA_CONFIG,
    MODEL_CONFIG,
    PATH_CONFIG,
    TRAINING_CONFIG,
)
from spectroscopy_qml.ir.mps_encoder_ttn.data_loader import load_ir_data, prepare_dataloaders
from spectroscopy_qml.ir.mps_encoder_ttn.model import (
...
def train_model(X: np.ndarray | None = None, y: np.ndarray | None = None):
    ...
    if MODEL_CONFIG.use_ttn_hybrid:
        model = MPSTTNHybridClassifier(
            input_dim=MODEL_CONFIG.input_dim,
            num_sites=MODEL_CONFIG.num_sites,
            physical_dim=MODEL_CONFIG.physical_dim,
            bond_dim=MODEL_CONFIG.bond_dim,
            ttn_internal_dim=MODEL_CONFIG.ttn_internal_dim,
            per_site_internal_dim=MODEL_CONFIG.per_site_internal_dim,
            num_classes=MODEL_CONFIG.num_classes,
            dropout_rate=MODEL_CONFIG.dropout_rate,
            freeze_encoder=MODEL_CONFIG.freeze_encoder,
            share_site_trees=MODEL_CONFIG.share_site_trees,
        )
        if MODEL_CONFIG.pretrained_encoder_path:
            model.load_encoder_weights(MODEL_CONFIG.pretrained_encoder_path, device)
    elif MODEL_CONFIG.use_ttn:
        model = MPSTTNClassifier(
            input_dim=MODEL_CONFIG.input_dim,
            num_sites=MODEL_CONFIG.num_sites,
            physical_dim=MODEL_CONFIG.physical_dim,
            bond_dim=MODEL_CONFIG.bond_dim,
    ...
    if MODEL_CONFIG.use_ttn or MODEL_CONFIG.use_ttn_hybrid:
        print(f"  TTN internal dim: {MODEL_CONFIG.ttn_internal_dim}")
        print(f"  Freeze encoder: {MODEL_CONFIG.freeze_encoder}")
        if MODEL_CONFIG.pretrained_encoder_path:
            print(f"  Pre-trained encoder: {MODEL_CONFIG.pretrained_encoder_path}")
    if MODEL_CONFIG.use_ttn_hybrid:
        print(f"  Per-site internal dim: {MODEL_CONFIG.per_site_internal_dim}")
        print(f"  Share site trees: {MODEL_CONFIG.share_site_trees}")
    ...
    with open(summary_path, "w") as f:
        ...
        if MODEL_CONFIG.use_ttn or MODEL_CONFIG.use_ttn_hybrid:
            f.write(f"  TTN internal dim: {MODEL_CONFIG.ttn_internal_dim}\n")
            f.write(f"  Freeze encoder: {MODEL_CONFIG.freeze_encoder}\n")
        if MODEL_CONFIG.use_ttn_hybrid:
            f.write(f"  Per-site internal dim: {MODEL_CONFIG.per_site_internal_dim}\n")
            f.write(f"  Share site trees: {MODEL_CONFIG.share_site_trees}\n")
