...
def main(model_path, data_dir, output_dir):
    ...
    # Recreate the same held-out split used during training.
    ...
    with open(summary_path, "w") as f:
        ...
        if "fold" in checkpoint:
            f.write(f"  Best fold: {checkpoint['fold']}\n")
        ...
        if "cv_metrics_mean" in checkpoint and "cv_metrics_std" in checkpoint:
            f.write("Cross-Validation Results (mean ± std across folds):\n")
            for metric_name, metric_value in checkpoint["cv_metrics_mean"].items():
                metric_std = checkpoint["cv_metrics_std"][metric_name]
                f.write(f"  {metric_name}: {metric_value:.4f} ± {metric_std:.4f}\n")
            f.write("\n")
