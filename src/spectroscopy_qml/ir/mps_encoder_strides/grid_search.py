...
from spectroscopy_qml.ir.mps_encoder_strides.config import (
    DATA_CONFIG,
    GRID_SEARCH_CONFIG,
    MODEL_CONFIG,
    PATH_CONFIG,
...
from spectroscopy_qml.ir.mps_encoder_strides.data_loader import load_ir_data
from spectroscopy_qml.ir.mps_encoder_strides.train import train_model
...
def _collect_grid() -> dict[str, tuple]:
    ...
    return {
        "segment_size": GRID_SEARCH_CONFIG.segment_size_values,
        "stride": GRID_SEARCH_CONFIG.stride_values,
...
def run_grid_search() -> None:
    ...
    for values in product(*(grid[key] for key in keys)):
        ...
        # Model constraint: (input_dim - segment_size) must be divisible by stride.
        input_dim = MODEL_CONFIG.input_dim
        seg = combo["segment_size"]
        st = combo["stride"]
        if seg > input_dim or (input_dim - seg) % st != 0:
            continue
    ...
    try:
        for idx, combo in enumerate(all_combinations, start=1):
            ...
            model_updates = {
                "segment_size": combo["segment_size"],
                "stride": combo["stride"],
