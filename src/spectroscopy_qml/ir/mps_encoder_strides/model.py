...
class MPSFunctionalGroupClassifier(nn.Module):
    ...
    1. Splits input into overlapping sites using a sliding window
    ...
    Args:
        ...
        segment_size: Size of each sliding window segment (default: 360)
        stride: Step between consecutive segments (default: 360)
    ...
    def __init__(
        ...
        assert segment_size <= input_dim, (
            f"segment_size ({segment_size}) must be <= input_dim ({input_dim})"
        )
        assert (input_dim - segment_size) % stride == 0, (
            f"(input_dim ({input_dim}) - segment_size ({segment_size})) "
        ...
        self.stride = stride
        self.num_sites = (input_dim - segment_size) // stride + 1
        self.physical_dim = physical_dim
        self.bond_dim = bond_dim
        ...
        self.feature_map = LocalFeatureMap(
            site_dim=self.segment_size, physical_dim=physical_dim, dropout_rate=dropout_rate
        )

        # MPS encoder
        self.mps_encoder = MPSEncoder(
            num_sites=self.num_sites, physical_dim=physical_dim, bond_dim=bond_dim, output_dim=128
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Split input into overlapping sites using unfold: (batch_size, num_sites, segment_size)
        x = x.unfold(1, self.segment_size, self.stride)

        # Apply local feature map to each site
        # Process all sites in parallel by reshaping
        x_flat = x.reshape(batch_size * self.num_sites, self.segment_size)
