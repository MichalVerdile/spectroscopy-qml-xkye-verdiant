...
class QuantumFeatureMap(nn.Module):
    ...
    For each scalar ``x``, this module computes:

    ``phi(x) = [cos(a * x_scaled), sin(a * x_scaled)]``

    Args:
        scale: Small-angle factor ``a`` used in the feature map.
        x_max_mode: Whether to compute ``x_max`` per sample or use a global value.
        global_x_max: Global scaling constant used when ``x_max_mode="global"``.
    ...
    def forward(self, x: Tensor, mask: Tensor | None = None) -> Tensor:
        ...
        Args:
            x: Tensor of any shape containing scalar intensities.
            mask: Optional mask with the same shape as ``x`` or broadcastable to it.

        Returns:
            Tensor with one extra trailing dimension of size 2.
        """
        ...
        # The paper scales inputs by x_max before the cosine-sine map so that the
        # embedding remains in a small-angle regime and behaves closer to the TTN paper.
...
class SegmentCompressor(nn.Module):
    ...
    Args:
        max_segment_length: Maximum padded length across all segments.
        chi: Bond dimension used throughout the tree.
    """
    ...
    def forward(self, embedded_segments: Tensor, segment_mask: Tensor | None = None) -> Tensor:
        ...
        Args:
            embedded_segments: Tensor of shape ``(batch, num_segments, max_segment_length, 2)``.
            segment_mask: Optional tensor of shape ``(num_segments, max_segment_length)``
                or ``(1, num_segments, max_segment_length, 1)`` marking valid entries.

        Returns:
            Tensor of shape ``(batch, num_segments, chi)``.
        """
...
class TensorMerge(nn.Module):
    ...
    Args:
        chi: Shared bond dimension of the tree vectors.
        use_bias: Whether to add a learnable bias after contraction.
    """
    ...
    def __init__(self, chi: int, use_bias: bool = True) -> None:
        ...
        # Small initialization keeps the bilinear TTN-inspired contraction stable at startup.
    ...
    def forward(self, left: Tensor, right: Tensor) -> Tensor:
        ...
        Args:
            left: Tensor of shape ``(..., chi)``.
            right: Tensor of shape ``(..., chi)``.
        ...
        # TTN-inspired bilinear contraction:
        # (..., chi), (..., chi), (chi, chi, chi) -> (..., chi)
        merged = torch.einsum("...i,...j,rij->...r", left, right, self.tensor)
        if self.bias is not None:
            merged = merged + self.bias

        # Repeated contractions can quickly change vector norms, so we renormalize
        # after every merge even though this is not a strict isometric TTN.
        return F.normalize(merged, dim=-1, eps=1e-8)


class TTNIRClassifier(nn.Module):
    """TTN-inspired multi-label classifier for IR spectra.

    The model:
    1. Splits each spectrum into contiguous segments.
    2. Applies a cosine-sine feature map to every scalar intensity.
    3. Pads shorter segments to a shared maximum length and compresses each segment to ``chi``.
    4. Builds a binary tree using tensor-based merge operators.
    5. Maps the root vector to multi-label logits.

    Args:
        num_labels: Number of output labels.
        chi: Bond dimension used for segment representations and tree nodes.
        num_segments: Number of contiguous segments. Must be a power of two.
        embedding_scale: Scale factor ``a`` used in the cosine-sine feature map.
        x_max_mode: Whether to compute ``x_max`` per sample or use a global value.
        global_x_max: Global scaling constant used when ``x_max_mode="global"``.
        use_bias: Whether the merge tensors use a learnable bias.
        input_dim: Expected spectrum length. Defaults to 1800.
    """
    ...
    def _segment_input(self, x: Tensor) -> Tensor:
        ...
        Returns:
            Tensor of shape ``(batch_size, num_segments, max_segment_length)``.
        """
        if x.ndim != 2:
    ...
    def forward(self, x: Tensor, apply_sigmoid: bool = False) -> Tensor:
        ...
        Args:
            x: Batch of spectra with shape ``(batch_size, input_dim)``.
            apply_sigmoid: If ``True``, apply sigmoid to the output logits.

        Returns:
            Tensor of shape ``(batch_size, num_labels)``.
        """
        # (batch, input_dim) -> (batch, num_segments, max_segment_length)
        segments = self._segment_input(x)

        # Scale by x_max before the cosine-sine map so the embedding stays close to
        # the small-angle regime used in the reference TTN paper.
        # The segment mask excludes padded positions from the per-sample x_max estimate.
        embedded_segments = self.feature_map(segments, mask=self.segment_mask.unsqueeze(0))

        # (batch, num_segments, max_segment_length, 2) -> (batch, num_segments, chi)
        ...
        # Binary reduction: 32 -> 16 -> 8 -> 4 -> 2 -> 1
        for level_index, merge in enumerate(self.merge_levels):
            ...
            left = node_states[:, 0::2, :]  # (batch, num_nodes / 2, chi)
            right = node_states[:, 1::2, :]  # (batch, num_nodes / 2, chi)
            node_states = merge(left, right)  # (batch, num_nodes / 2, chi)
