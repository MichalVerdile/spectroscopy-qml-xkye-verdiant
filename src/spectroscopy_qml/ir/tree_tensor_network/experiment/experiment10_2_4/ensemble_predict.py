"""Experiment 10.2.4: Ensemble evaluation — TTN 10.2 + 1D-CNN Specialist Heads."""
...
from spectroscopy_qml.ir.tree_tensor_network.experiment.experiment10_2_4.model import (  # noqa: E402
    TTN102CNNEnsemble,
    load_ttn102,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Ensemble evaluation: TTN 10.2 + 1D-CNN Specialist Heads (Exp 10.2.4)."
    )
...
def main():
    ...
    print("Loading CNN specialist ensemble...")
    spec_config = json.loads((args.specialist_dir / "run_config.json").read_text())
    ensemble = TTN102CNNEnsemble(
        ttn=ttn,
        specialist_indices=specialist_indices,
        cnn_channels=spec_config.get("cnn_channels", 32),
        pool_size=spec_config.get("pool_size", 8),
        dropout=0.0,
    ).to(device)
    ...
    print_metrics("CNN Specialist Heads alone (10 hard classes)", spec_metrics, specialist_indices)
    ...
    print(f"\nSelective override {len(override_indices)}/10: "
    ...
    print_metrics("Ensemble: TTN 10.2 + CNN Specialist Override", ens_metrics)

    ens_per = ens_metrics["per_class_f1"]
    print("\n--- Specialist classes detail ---")
    print(f"{'Class':20s} | {'TTN 10.2':>10} | {'CNN Head':>10} | {'Ensemble':>10} | Override")
    print("-" * 72)
    for local_i, global_i in enumerate(specialist_indices):
