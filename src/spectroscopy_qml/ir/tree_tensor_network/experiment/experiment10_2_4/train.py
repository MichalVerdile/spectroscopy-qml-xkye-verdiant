"""Experiment 10.2.4: Train 1D-CNN specialist heads on diagnostic spectral windows."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train 1D-CNN specialist heads (Exp 10.2.4)."
    )
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=Path("src/spectroscopy_qml/ir/tree_tensor_network"
                                     "/experiment/experiment10_2_4/results"))
    ...
    parser.add_argument("--cnn-channels", type=int, default=32,
                        help="Base channels for the 1D CNN (doubled in second layer).")
...
def write_summary(path, *, specialist_names, best_epoch, best_score,
    ...
    lines = [
        "Experiment 10.2.4 — TTN 10.2 + 1D-CNN Specialist Head Summary",
        "=" * 80,
        f"Specialist classes:    {', '.join(specialist_names)}",
        f"CNN channels:          {args.cnn_channels} (x2 in second layer)",
