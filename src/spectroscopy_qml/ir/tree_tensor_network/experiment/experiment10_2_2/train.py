"""Experiment 10.2.2: Train combined TTN features + spectral window specialist heads.

Frozen TTN 10.2 backbone (64-dim readout) + per-class spectral window projection
→ MLP binary head. Oversampling via WeightedRandomSampler on rare positives.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train combined specialist heads: TTN features + spectral windows (Exp 10.2.2)."
    )
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=Path("src/spectroscopy_qml/ir/tree_tensor_network"
                                    "/experiment/experiment10_2_2/results"))
    ...
    parser.add_argument("--hidden-dims", type=parse_hidden_dims, default=[128, 64],
                        help="MLP hidden dims, e.g. '128,64'.")
    ...
    parser.add_argument("--oversample", action="store_true", default=True,
                        help="Use WeightedRandomSampler to oversample rare positives.")
    ...
    parser.add_argument("--oversample-epoch-multiplier", type=float, default=1.0,
                        help="Num samples per epoch = multiplier * train_size.")
...
def make_oversampled_weights(labels: np.ndarray) -> np.ndarray:
    """Per-sample weight = max inverse-frequency among its positive labels.

    Samples with no positive label get the minimum non-zero weight.
    """
    n, c = labels.shape
    class_pos_counts = labels.sum(axis=0).clip(min=1)
    class_weights = n / class_pos_counts  # higher weight for rarer classes
...
def write_summary(path, *, specialist_names, best_epoch, best_score,
    ...
    lines = [
        "Experiment 10.2.2 — TTN 10.2 Combined Specialist Head Summary",
        "=" * 80,
...
def main():
    ...
    if args.oversample:
        sw = make_oversampled_weights(train_labels)
        print(f"Oversampling: weight min={sw.min():.1f}, max={sw.max():.1f}, "
              f"samples/epoch={int(len(train_labels)*args.oversample_epoch_multiplier)}")
