"""Experiment 10.2.2: Ensemble evaluation — TTN 10.2 + Combined Specialist Heads.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Ensemble evaluation: TTN 10.2 + Combined Specialist Heads (Exp 10.2.2)."
    )
    parser.add_argument("--specialist-dir", type=Path, required=True,
                        help="Output dir of a completed experiment10_2_2 training run.")
...
def main():
    ...
    print("Loading combined specialist ensemble...")
    ...
    ttn_probs, ttn_labels = get_probs_ttn(ttn, test_loader, device)  # (N, 37)
    ...
    # --- 2. Specialist heads alone (10 classes) ---
    print("\nRunning specialist head predictions...")
    spec_probs  = get_probs_specialist(ensemble, test_loader, device)  # (N, 10)
    ...
    print_metrics("Combined Specialist Heads alone (10 hard classes)", spec_metrics, ALL_LABEL_NAMES, specialist_indices)

    ttn_per  = ttn_metrics["per_class_f1"]
    spec_per = spec_metrics["per_class_f1"]

    # --- 3. Ensemble: selective specialist override ---
    override_indices = [
        (local_i, global_i)
