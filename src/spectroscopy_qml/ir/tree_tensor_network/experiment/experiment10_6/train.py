"""Training entry point for experiment 10.6 - OPTIMIZED TTN with extended training and weighted loss."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train experiment10.6 (OPTIMIZED): TTN IR classifier with Lorentzian-smoothed feature "
            "channels, extended training (300 epochs), enhanced threshold optimization, "
            "and weighted class balancing for rare functional groups (Thial, Azo, Hydrazone)."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("src/spectroscopy_qml/ir/tree_tensor_network/experiment/experiment10_6/results"),
    )
    ...
    parser.add_argument("--epochs", type=int, default=300, help="Extended from 200 to 300 epochs for better convergence")
    ...
    parser.add_argument("--pos-weight-power", type=float, default=0.6, help="Increased from 0.5 for stronger class balancing on rare groups")
    ...
    parser.add_argument("--early-stopping-patience", type=int, default=40, help="Doubled from 20 to 40 for extended training")
    parser.add_argument("--early-stopping-min-delta", type=float, default=5e-5, help="More sensitive stopping criteria: 5e-5 instead of 1e-4")
    parser.add_argument("--min-epochs-before-stopping", type=int, default=20, help="Reduced from 30 to 20 for faster early learning")
...
def describe_args(args: argparse.Namespace, split_path: Path) -> None:
    print("=" * 80)
    print("TTN IR Experiment10.6 (OPTIMIZED) Training")
    print("=" * 80)
    print("📊 Configuration: Extended training + weighted class balancing + enhanced thresholds")
    print("")
    ...
    print(f"Early stop patience:      {args.early_stopping_patience} (INCREASED from 20)")
    print(f"Min epochs before stop:   {args.min_epochs_before_stopping} (DECREASED from 30)")
    print(f"Min delta for stopping:   {args.early_stopping_min_delta} (MORE SENSITIVE)")
    print(f"Loss type:                {args.loss_type}")
    print(f"Pos weight power:         {args.pos_weight_power} (INCREASED from 0.5)")
