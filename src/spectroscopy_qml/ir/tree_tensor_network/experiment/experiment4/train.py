"""Training entry point for the paper-nearer TTN classifier."""
...
from model import TTNIRClassifier  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train the paper-nearer TTN-inspired IR classifier."
    )
    parser.add_argument("--data-dir", type=Path, default=Path("data/raw"))
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("src/spectroscopy_qml/ir/tree_tensor_network/experiment/experiment4/results"),
    )
    ...
    parser.add_argument("--threshold-metric", choices=["f1_micro", "f1_macro", "per_class_f1"], default="f1_micro")
...
def describe_args(args: argparse.Namespace) -> None:
    print("=" * 80)
    print("TTN IR Experiment4 Training")
    print("=" * 80)
...
def run_synthetic_preflight(model: TTNIRClassifier, args: argparse.Namespace, device: torch.device) -> None:
    ...
    labels = torch.randint(0, 2, (args.preflight_batch_size, args.num_labels), device=device, dtype=torch.float32)
...
def write_summary(
    ...
    with summary_path.open("w") as handle:
        handle.write("TTN IR Experiment4 Summary\n")
        handle.write("=" * 80 + "\n")
