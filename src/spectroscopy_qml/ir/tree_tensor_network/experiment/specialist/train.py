"""Training entry point for specialist model - 10 rare functional groups."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train specialist: TTN IR classifier for 10 rare functional groups. "
            "Works in ensemble with experiment10_2 baseline for final predictions."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("src/spectroscopy_qml/ir/tree_tensor_network/experiment/specialist/results"),
    )
    parser.add_argument("--split-path", type=Path, default=None)
    parser.add_argument("--overwrite-split", action="store_true")
    parser.add_argument("--input-dim", type=int, default=1800)
    # SPECIALIST: Only 10 rare functional groups instead of all 37
    SPECIALIST_GROUPS = 10
    parser.add_argument("--num-labels", type=int, default=SPECIALIST_GROUPS)
    ...
    parser.add_argument("--epochs", type=int, default=200, help="Specialist typically stops earlier due to early stopping")
    parser.add_argument("--learning-rate", type=float, default=3e-4)
    parser.add_argument("--specialist-label-indices", type=str, default="34,10,19,27,33,23,24,25,29,36", help="Indices of 10 rare FGs")
...
def describe_args(args: argparse.Namespace, split_path: Path) -> None:
    print("=" * 80)
    print("TTN Specialist Model Training - 10 Rare Functional Groups")
    print("=" * 80)
    print("🔬 Designed to work with Experiment 10.2 baseline in ensemble mode")
    print("")
...
def main() -> None:
    ...
    # SPECIALIST: Filter to only 10 rare functional groups
    specialist_indices = [int(x) for x in args.specialist_label_indices.split(',')]
    print(f"\n🔬 SPECIALIST MODE: Filtering to {len(specialist_indices)} rare functional groups")
    print(f"   Label indices: {specialist_indices}")
    specialist_names = [
        "Thial", "Azo compound", "Hydrazone", "Phosphine", "Sulfoxide",
        "Acid anhydride", "Imine", "Enamine", "Acyl halide", "Sulfide"
    ]
    print(f"   Groups: {specialist_names}")
    
    y_full = y.copy()  # Keep original
    y = y[:, specialist_indices]  # Filter to specialist groups
    print(f"   Original labels shape: {y_full.shape} → Specialist shape: {y.shape}")
    
    if X.shape[1] != args.input_dim:
        raise RuntimeError(f"Loaded spectra have width {X.shape[1]}, expected {args.input_dim}.")
    if y.shape[1] != 10:
        raise RuntimeError(f"Specialist expects 10 labels, got {y.shape[1]}.")
