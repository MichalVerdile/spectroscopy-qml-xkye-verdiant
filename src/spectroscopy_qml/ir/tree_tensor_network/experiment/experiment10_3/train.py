"""Training entry point for experiment 10.3."""
...
from spectroscopy_qml.ir.tree_tensor_network.experiment.experiment10_3.model import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train experiment10.3: TTN IR classifier with Voigt-profile-smoothed feature "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("src/spectroscopy_qml/ir/tree_tensor_network/experiment/experiment10_3/results"),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help="Lorentzian HWHM of the Voigt kernel in data points.",
    )
    parser.add_argument(
        "--voigt-gamma-g",
        type=float,
        default=2.0,
        help="Gaussian sigma of the Voigt kernel in data points.",
    )
    parser.add_argument(
        "--voigt-eta",
        type=float,
        default=0.5,
        help="Mixing factor: 0=pure Gaussian, 1=pure Lorentzian.",
    )
    parser.add_argument(
        "--voigt-kernel-half-width",
        type=int,
        default=20,
        help="Kernel half-width in data points.",
    )
    parser.add_argument(
        "--voigt-norm-mode", choices=["max_abs", "z_score", "percentile"], default="max_abs",
...
def build_model(args: argparse.Namespace) -> TTNIRClassifier10_3:
    return TTNIRClassifier10_3(
        ...
        voigt_norm_mode=args.voigt_norm_mode,
    )


def describe_args(args: argparse.Namespace, split_path: Path) -> None:
    print("=" * 80)
    print("TTN IR Experiment10.3 Training")
    print("=" * 80)
    ...
    print(f"Feature channels:         raw + voigt_d1 + voigt_d2")
    print(f"Voigt gamma_l (Lorentz):  {args.voigt_gamma_l}")
    print(f"Voigt gamma_g (Gaussian): {args.voigt_gamma_g}")
    print(f"Voigt eta (mix):          {args.voigt_eta}  (0=Gauss, 1=Lorentz)")
    print(f"Voigt kernel half-width:  {args.voigt_kernel_half_width}")
