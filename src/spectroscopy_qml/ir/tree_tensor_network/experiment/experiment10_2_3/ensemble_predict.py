"""Experiment 10.2.3: Ensemble evaluation — TTN 10.2 (partial fine-tune) + Specialist Heads.

Compares three configurations on the test set:
  1. TTN 10.2 alone (all 37 classes)
  2. Combined specialist heads alone (10 hard classes)
  3. Ensemble: TTN 10.2 + selective specialist override for the 10 hard classes
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Ensemble evaluation: TTN 10.2 partial fine-tune + Specialist Heads (Exp 10.2.3)."
    )
...
@torch.no_grad()
def get_probs_ttn(model, loader, device):
    model.eval()
...
@torch.no_grad()
def get_probs_specialist(model, loader, device):
    model.eval()
    all_probs = []
...
def print_metrics(label, metrics, label_names, indices=None):
    print(f"\n{'='*60}")
    print(f"  {label}")
    print(f"{'='*60}")
...
def main():
    ...
    print("Loading specialist ensemble...")
    spec_config = json.loads((args.specialist_dir / "run_config.json").read_text())
    ensemble = TTN102CombinedEnsemble(
        ttn=ttn,
        specialist_indices=specialist_indices,
        hidden_dims=spec_config.get("hidden_dims", [128, 64]),
        window_proj_dim=spec_config.get("window_proj_dim", 32),
        dropout=0.0,
        finetune_layers=spec_config.get("finetune_ttn_layers", 1),
    ).to(device)
    ...
    ttn_probs, ttn_labels = get_probs_ttn(ttn, test_loader, device)
    ...
    # --- 2. Specialist heads alone ---
    print("\nRunning specialist head predictions...")
    spec_probs  = get_probs_specialist(ensemble, test_loader, device)
    ...
    print_metrics("Specialist Heads alone (10 hard classes)", spec_metrics, ALL_LABEL_NAMES, specialist_indices)

    ttn_per  = ttn_metrics["per_class_f1"]
    spec_per = spec_metrics["per_class_f1"]

    # --- 3. Ensemble: selective override ---
    override_indices = [
        (local_i, global_i)
