...
class MPSFunctionalGroupClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.num_sites = num_sites
        self.base_site_dim = input_dim // num_sites
        # Each site gets 4 channels: original + 1st derivative + 2nd derivative + integral
        self.num_channels = 4
        self.site_dim = self.base_site_dim * self.num_channels
        self.physical_dim = physical_dim
        self.bond_dim = bond_dim
    ...
    def _compute_spectral_features(self, x: torch.Tensor) -> torch.Tensor:
        """
        Enhance spectra with 1st/2nd derivatives and cumulative integral.

        Args:
            x: Input tensor of shape (batch_size, input_dim)

        Returns:
            Enhanced tensor of shape (batch_size, num_sites, site_dim)
            where site_dim = base_site_dim * 4
        """
        # First derivative (finite differences), pad to keep length
        d1 = torch.diff(x, n=1, dim=1)
        d1 = torch.nn.functional.pad(d1, (0, 1), mode="replicate")

        # Second derivative
        d2 = torch.diff(x, n=2, dim=1)
        d2 = torch.nn.functional.pad(d2, (0, 2), mode="replicate")

        # Cumulative integral (trapezoidal-like via cumsum)
        integral = torch.cumsum(x, dim=1)

        # Normalize each channel independently so no channel dominates
        ...
        d1 = _normalize(d1)
        d2 = _normalize(d2)
        integral = _normalize(integral)

        # Stack channels: (batch_size, 4, input_dim)
        enhanced = torch.stack([x_norm, d1, d2, integral], dim=1)

        # Reshape to (batch_size, 4, num_sites, base_site_dim)
        enhanced = enhanced.view(
            x.size(0), self.num_channels, self.num_sites, self.base_site_dim
        )

        # Permute to (batch_size, num_sites, 4, base_site_dim) then flatten channels
        enhanced = enhanced.permute(0, 2, 1, 3).contiguous()
        enhanced = enhanced.view(x.size(0), self.num_sites, self.site_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Enhance spectra with derivatives and integral
        # Result: (batch_size, num_sites, site_dim) where site_dim = base_site_dim * 4
        x = self._compute_spectral_features(x)
