...
class MPSFunctionalGroupClassifier(nn.Module):
    ...
    def __init__(
        ...
        # Second MPS config (only used when use_second_encoder=True)
        self.num_sites_2 = num_sites_2
        self.site_dim_2 = input_dim // num_sites_2 if use_second_encoder else 0
        ...
        if classifier_head == "cnn":
            ...
            extra_dim = 256 if use_second_encoder else 0
            self.fc1 = nn.Linear(flat_size + extra_dim, 4927)
            ...
            combined_dim = 2 * intermediate_dim if use_second_encoder else intermediate_dim
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        features = self.feature_map(x_flat).view(batch_size, self.num_sites, self.physical_dim)

        # === Second MPS (optional) ===
        if self.use_second_encoder:
            x_flat_2 = x.view(batch_size * self.num_sites_2, self.site_dim_2)
            features_2 = self.feature_map_2(x_flat_2).view(
                batch_size, self.num_sites_2, self.physical_dim_2
            )
        ...
        per_site = torch.cat([torch.cat([fw, bw], dim=2), x_sites], dim=2)
        ...
        x = self.relu(self.cnn_dropout(self.fc1(x)))
        x = self.relu(self.cnn_dropout(self.fc2(x)))
        x = self.relu(self.cnn_dropout(self.fc3(x)))
