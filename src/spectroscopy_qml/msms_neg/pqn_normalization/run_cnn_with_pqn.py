...
# Modified to include PQN (Probabilistic Quotient Normalization)
...
def apply_pqn_normalization_single_group(spectra, eps=1e-8):
    """
    Apply PQN normalization to a single group of spectra.
    ...
    # Step 2: reference spectrum as median profile within this group
...
def apply_pqn_normalization(
    spectra,
    cluster_labels=None,
    eps=1e-8,
    min_cluster_size_for_pqn: int = 20,
):
    """
    Apply Probabilistic Quotient Normalization (PQN) across spectra.
    If cluster_labels provided, applies PQN separately to each cluster group.

    Args:
        spectra: 2D numpy array (n_samples, n_features)
        cluster_labels: Optional array of cluster labels for grouped normalization
        eps: Small value for numerical stability
        min_cluster_size_for_pqn: Minimum cluster size required to apply grouped PQN
    ...
    if cluster_labels is None:
        # Apply PQN to all spectra together (original behavior)
        return apply_pqn_normalization_single_group(spectra, eps)

    # Apply PQN separately to each cluster group
    ...
    for cluster_id in unique_clusters:
        ...
        if len(group_spectra) >= min_cluster_size_for_pqn:
            # Apply PQN within this group
            normalized[mask] = apply_pqn_normalization_single_group(group_spectra, eps)
            normalized_cluster_count += 1
        else:
            # Small groups fall back to simple TIC normalization.
            row_sums = group_spectra.sum(axis=1, keepdims=True)
            safe_row_sums = np.where(row_sums > eps, row_sums, 1.0)
            normalized[mask] = group_spectra / safe_row_sums
            skipped_cluster_count += 1

    print(
        "Grouped PQN normalization summary: "
        f"normalized_clusters={normalized_cluster_count}, "
        f"skipped_clusters={skipped_cluster_count}, "
        f"min_cluster_size_for_pqn={min_cluster_size_for_pqn}"
    )
...
@click.option(
    "--min-cluster-size-for-pqn",
    type=int,
    default=20,
    show_default=True,
    help="Minimum cluster size required to apply grouped PQN",
)
@click.option(
    "--min-bucket-size",
    type=int,
...
def main(
    ...
    fingerprint_bits,
    tanimoto_threshold,
    min_cluster_size_for_pqn,
    min_bucket_size,
    max_bucket_size,
):
    if not (0.0 < tanimoto_threshold <= 1.0):
        raise click.BadParameter("--tanimoto-threshold must be in (0, 1].")
    if min_cluster_size_for_pqn < 1:
        raise click.BadParameter("--min-cluster-size-for-pqn must be >= 1.")
    ...
    cluster_labels = relabel_small_clusters_as_unique(
        cluster_labels,
        min_cluster_size=min_cluster_size_for_pqn,
    )
    print("Grouping complete and small clusters relabeled for safe PQN fallback.")
    ...
    for col_name in columns_to_process:
        ...
        print(f"Output directory: {output_dir}_pqn")
        print(f"{'='*60}")

        # Prepare data and apply PQN normalization per group
        X_data = np.stack(training_data[actual_col].to_list()).astype(np.float32, copy=False)
        X_data = apply_pqn_normalization(
            X_data,
            cluster_labels=cluster_labels,
            min_cluster_size_for_pqn=min_cluster_size_for_pqn,
        )
        print(f"Applied PQN normalization with Tanimoto grouping to {actual_col}")
        ...
        print(f"\n{'='*60}")
        print(f"Cross-Validation Results for {col_name} (with PQN):")
        ...
        print(f"\n{'='*60}")
        print(f"FINAL TEST RESULTS for {col_name} (with PQN):")
        ...
        # Save the best model
        model_save_path = model_dir / f"{output_dir}_pqn_model.keras"
        ...
        with open(summary_path, "w") as f:
            f.write(f"CNN Model with PQN Normalization - {col_name.upper()}\n")
            f.write(f"{'='*60}\n\n")
            f.write("Model Configuration:\n")
            f.write("  - Normalization: PQN\n")
            ...
            f.write(f"  - Min Cluster Size for PQN: {min_cluster_size_for_pqn}\n")
