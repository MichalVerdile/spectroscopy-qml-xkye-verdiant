...
def multilabel_train_test_split(
    ...
    """Split arrays with iterative stratification for multilabel targets."""
...
def load_ir_data(
    ...
    for i, parquet_file in enumerate(parquet_files):
        ...
        df = pd.read_parquet(parquet_file, columns=["msms_negative_40ev", "smiles"])
        ...
        df = df[df["msms_negative_40ev"].notna()]

        spectra = np.stack(
            [make_msms_spectrum(s) for s in df["msms_negative_40ev"]]
        )

        # Interpolate spectra
        spectra = np.stack(
            [interpolate_spectrum(spec, target_length) for spec in spectra]
        )
...
def prepare_dataloaders(
    ...
    # First split: separate test set while preserving multilabel frequencies.
    X_temp, X_test, y_temp, y_test = multilabel_train_test_split(
        X,
        y,
    ...
    # Second split: separate train and validation while preserving multilabel frequencies.
