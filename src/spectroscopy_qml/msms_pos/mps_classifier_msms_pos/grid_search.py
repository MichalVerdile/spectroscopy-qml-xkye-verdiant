...
from spectroscopy_qml.msms_pos.mps_classifier_msms_pos.config import (
    DATA_CONFIG,
    GRID_SEARCH_CONFIG,
    MODEL_CONFIG,
    PATH_CONFIG,
...
from spectroscopy_qml.msms_pos.mps_classifier_msms_pos.data_loader import load_ir_data
from spectroscopy_qml.msms_pos.mps_classifier_msms_pos.train import train_model
...
def _extract_score(result: dict) -> float:
    ...
    if metric_name.startswith("val_"):
        ...
        if "cv_val_metrics" in result:
            return float(result["cv_val_metrics"][key])
    ...
    try:
        for idx, combo in enumerate(all_combinations, start=1):
            ...
            row = {
                ...
                "cv_val_loss_mean": result.get("cv_val_loss_mean"),
                "cv_val_loss_std": result.get("cv_val_loss_std"),
                ...
                "val_f1_micro": result.get("cv_val_metrics", result["best_val_metrics"])["f1_micro"],
                "val_f1_macro": result.get("cv_val_metrics", result["best_val_metrics"])["f1_macro"],
