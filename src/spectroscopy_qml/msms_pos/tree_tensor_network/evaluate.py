...
def _resolve_split_path(run_config: dict, run_dir: Path, data_dir: Path) -> Path:
    ...
    return run_dir / f"data_split_seed_msms_pos{int(run_config['seed'])}_{split_suffix}.npz"
...
@click.option(
    "--output_dir",
    type=click.Path(path_type=Path),
    default=Path("src/spectroscopy_qml/msms_pos/tree_tensor_network/results_600"),
