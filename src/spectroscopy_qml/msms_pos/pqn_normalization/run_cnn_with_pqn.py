...
def apply_pqn_normalization(spectra, cluster_labels=None, eps=1e-8):
    """
    ...
    for cluster_id in unique_clusters:
        ...
        if len(group_spectra) >= 2:
            ...
            # Single sample - just TIC normalize
            row_sum = group_spectra.sum()
            if row_sum > eps:
                normalized[mask] = group_spectra / row_sum
            else:
...
def main(analytical_data, base_out_path, columns, seed, n_folds):
    ...
    for col_name in columns_to_process:
        ...
        X_data = apply_pqn_normalization(X_data, cluster_labels=cluster_labels)
        ...
        with open(summary_path, "w") as f:
            ...
            f.write("  - Normalization: PQN with Tanimoto grouping\n")
            f.write(
                "  - Clustering: Agglomerative (50 clusters based on Morgan fingerprint similarity)\n"
            )
