...
def load_ir_data(
    ...
    for i, parquet_file in enumerate(parquet_files):
        ...
        df = pd.read_parquet(parquet_file, columns=["h_nmr_spectra", "smiles"])
        ...
        df = df[df["h_nmr_spectra"].notna()]

        # Interpolate spectra
        spectra = np.stack(
            [interpolate_spectrum(spec, target_length) for spec in df["h_nmr_spectra"].values]
        )
