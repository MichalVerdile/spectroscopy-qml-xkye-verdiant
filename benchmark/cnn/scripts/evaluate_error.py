"""Evaluation script for the CNN IR functional-group model.
...
It evaluates a saved Keras CNN model produced by the CNN training script.
"""
...
def _load_cnn_model(model_path: Path):
    # compile=False avoids needing the custom weighted-loss function used during training.
    return load_model(str(model_path), compile=False)


def _predict_probabilities(model, X: np.ndarray, batch_size: int) -> np.ndarray:
    X_reshaped = X.reshape(X.shape[0], 600, 1)
    return np.asarray(model.predict(X_reshaped, batch_size=batch_size, verbose=1), dtype=np.float32)
...
def _resolve_model_path(results_dir: Path, output_short_name: str) -> Path:
    preferred = results_dir / f"{output_short_name}_model.keras"
    if preferred.exists():
        return preferred
    candidates = sorted(results_dir.glob("*.keras"))
    if len(candidates) == 1:
        return candidates[0]
    if not candidates:
        raise FileNotFoundError(f"No .keras model found in {results_dir}")
    raise ValueError(f"Multiple .keras models found in {results_dir}; pass --model_path explicitly.")
...
def _evaluate_from_saved_binary_predictions(payload: dict, mode: str) -> tuple[dict, dict | None, dict] | None:
    ...
    0.5-threshold test metrics is still possible.

    Validation data is optional in this compatibility mode. If validation targets
    and predictions are missing, test artifacts are still produced and validation
    artifacts are skipped with an explicit note in the summary.
    """
    if not payload:
        return None
    ...
    # Be mode-agnostic here. Some old runs are later evaluated with a different
    # CLI --mode value, but k-fold pickles still contain all_cv_* arrays.
    ...
    # Saved predictions from the training scripts are already binary 0/1 labels.
    # Threshold at 0.5 only to reuse the same metric/error-analysis implementation.
    ...
    val_results = None
    val_size = None
    if val_y is not None and val_pred is not None:
    ...
    if val_results is None:
        split_info["validation_note"] = "No saved validation predictions/targets were found; validation artifacts were skipped."
...
def _prepare_splits(
    ...
    # Prefer the split metadata from results.pickle. This avoids the old bug where
    # training used seed=3245 but evaluation silently reconstructed with seed=42.
    if payload:
    ...
    if saved_X_test is not None and saved_y_test is not None:
        X_test = saved_X_test.astype(np.float32)
        y_test = saved_y_test.astype(np.int32)
        if X_test.ndim == 3 and X_test.shape[-1] == 1:
            X_test = X_test.reshape(X_test.shape[0], X_test.shape[1])

        if saved_X_val is not None and saved_y_val is not None:
            X_val = saved_X_val.astype(np.float32)
            y_val = saved_y_val.astype(np.int32)
...
@click.option("--base_out_path", type=click.Path(path_type=Path), default="benchmark/cnn/results_600", help="Base output path used by the training script.")
@click.option("--results_dir", type=click.Path(path_type=Path), default=None, help="Exact CNN output directory, e.g. .../ir/original or .../ir/k_fold.")
...
@click.option("--seed", type=int, default=None, show_default=True, help="Seed used for split reconstruction. Defaults to the seed stored in results.pickle when available.")
@click.option("--n_folds", type=int, default=None, show_default=True, help="K-fold count. Defaults to n_folds stored in results.pickle when available.")
@click.option("--batch_size", type=int, default=1024, show_default=True)
def main(
    ...
    # If this is an old results.pickle that contains targets/predictions but not
    # exact X arrays, using saved predictions is the only exact option. It also
    # avoids spending minutes loading parquet files only to discover that their
    # current order cannot reproduce the historical split.
    ...
    if saved_binary_evaluation is not None and not has_exact_saved_arrays:
        print(
            ...
            "Validation artifacts are generated only if validation predictions/targets exist in the pickle.\n"
        ...
        try:
            ...
            print(f"Model loaded with {parameter_count:,} parameters")

            print("\n" + "=" * 80)
            print("Evaluating on Test Set")
            print("=" * 80)
            test_prob = _predict_probabilities(model, X_test, batch_size=batch_size)
            test_results = evaluate_arrays(y_test, test_prob, thresholds=thresholds)

            print("\n" + "=" * 80)
            print("Evaluating on Validation Set")
            print("=" * 80)
            val_prob = _predict_probabilities(model, X_val, batch_size=batch_size)
    ...
    test_csv_path, test_metrics_path, test_plot_path, test_top_plot_path = save_error_analysis_artifacts(
        test_results, functional_group_names, results_dir, "test", "CNN IR Test Set", "#4ECDC4"
    )

    val_output_path = results_dir / "validation_results.txt"
    val_csv_path = val_metrics_path = val_plot_path = val_top_plot_path = None
    if val_results is not None:
        ...
        val_csv_path, val_metrics_path, val_plot_path, val_top_plot_path = save_error_analysis_artifacts(
            val_results, functional_group_names, results_dir, "validation", "CNN IR Validation Set", "#FF6B6B"
        )
    else:
        val_output_path.write_text(
            "Validation evaluation was skipped because this old results.pickle does not contain "
            "saved validation predictions/targets and does not contain exact X_val/y_val arrays for safe re-inference.\n",
            encoding="utf-8",
        )
    ...
    with summary_path.open("w", encoding="utf-8") as handle:
        handle.write("=" * 80 + "\n")
        handle.write("CNN IR - Evaluation Summary\n")
        handle.write("=" * 80 + "\n\n")
        ...
        handle.write(f"  Parameters: {parameter_count:,}\n")
        ...
        if val_results is not None:
            ...
            handle.write("  skipped: no saved validation predictions/targets were available in this old results.pickle, and exact X_val/y_val arrays were not available for safe re-inference.\n")
        ...
        if val_results is not None:
            ...
            handle.write("  Validation artifacts: skipped; see validation_results.txt for explanation.\n")
    ...
    if val_results is not None:
        ...
        print("Validation error-analysis artifacts skipped because validation data was unavailable in the old pickle.")
    print("\n" + "=" * 80)
