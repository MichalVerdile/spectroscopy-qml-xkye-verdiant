"""Detailed error analysis for FunctionalGroup XGBoost MultiOutputClassifier models.

This script mirrors the TTN/CNN error-analysis style, but is adapted for the
XGBoost MultiOutputClassifier training script.

It supports two workflows:

1) Fast artifact-based analysis, recommended when you already have results.pickle:
   - original mode results.pickle with keys: pred, tgt
   - k_fold mode results.pickle with keys: test_predictions, test_targets
   - if probability arrays were saved later, keys such as prob, test_probabilities,
     train_prob, val_prob are also supported.

2) Full reload + inference, useful when you want probabilities, threshold tuning,
   another split, or you only have the saved XGBoost model pickle. In this mode
   the script rebuilds the same data split from parquet files and runs the model.

Outputs follow the same style as the CNN/TTN analyzer:
...
python scripts/analyse_xgboost_errors.py \
  --results-pickle benchmark/xgboost/models_whole/ir/original/results.pickle \
  --output-dir benchmark/xgboost/results_whole/ir/original/evaluation

Analyze k-fold results.pickle directly:

python scripts/analyse_xgboost_errors.py \
  --results-pickle benchmark/xgboost/models_whole/ir/k_fold/results.pickle \
  --output-dir benchmark/xgboost/results_whole/ir/k_fold/evaluation

Analyze from a saved model and parquet data:

python scripts/analyse_xgboost_errors.py \
  --analytical-data data/raw \
  --model-checkpoint benchmark/xgboost/models_600/cnmr/original/cnmr_xgboost_model.pickle \
  --column c_nmr_spectra \
  --mode original \
  --split test \
  --output-dir benchmark/xgboost/results_600/cnmr/eval
"""
...
# -----------------------------------------------------------------------------
# Data preprocessing, kept compatible with the XGBoost training script.
# -----------------------------------------------------------------------------
...
def interpolate_to_600(spec) -> np.ndarray | None:
    """Interpolate any 1D spectrum/vector to exactly 600 values."""
    if spec is None:
        return None
    ...
    return interp(new_x).astype(np.float32)
...
def load_column_data(
    ...
    """Load spectra and labels like the XGBoost training script.

    Returns:
        X: shape (n_samples, 600)
...
def rebuild_splits(
    ...
    """Recreate train/val/test splits from the XGBoost training script."""
...
# -----------------------------------------------------------------------------
# Prediction helpers for MultiOutputClassifier/XGBoost
# -----------------------------------------------------------------------------


def load_xgboost_model(checkpoint_path: Path) -> Any:
    with checkpoint_path.open("rb") as handle:
        return pickle.load(handle)


def predict_xgboost_probs(model: Any, X: np.ndarray) -> np.ndarray:
    """Return probability matrix (n_samples, n_labels) for MultiOutputClassifier.

    MultiOutputClassifier.predict_proba returns a list of arrays, one per label.
    Each array usually has shape (n_samples, 2), but if a label had one class only
    during fitting, sklearn may return one probability column. This function is
    defensive and returns a usable positive-class probability for every label.
    """
    ...
    if isinstance(raw, list):
        columns = []
        estimators = getattr(model, "estimators_", [None] * len(raw))
        for label_idx, arr in enumerate(raw):
            ...
            if arr.shape[1] == 2:
                ...
                continue
            if arr.shape[1] == 1:
                classes = getattr(estimators[label_idx], "classes_", np.array([1]))
                positive_prob = arr[:, 0] if len(classes) and int(classes[0]) == 1 else np.zeros(arr.shape[0], dtype=np.float32)
                columns.append(positive_prob)
                continue
            columns.append(arr[:, -1])
        probs = np.vstack(columns).T
    else:
...
def predict_xgboost_labels(model: Any, X: np.ndarray) -> np.ndarray:
    return np.asarray(model.predict(X)).astype(np.int32)
...
# -----------------------------------------------------------------------------
# Analysis tables, same semantics as CNN/TTN analyzer.
# -----------------------------------------------------------------------------
...
def _first_existing(results: dict[str, Any], keys: list[str]) -> Any | None:
    for key in keys:
        if key in results:
            return results[key]
    return None
...
def load_bundle_from_results_pickle(
    results_pickle: Path,
    split: str = "test",
) -> tuple[PredictionBundle, np.ndarray | None, dict[str, Any]]:
    with results_pickle.open("rb") as handle:
        results = pickle.load(handle)

    thresholds = _first_existing(results, ["best_thresholds", "thresholds"])
    thresholds_np = np.asarray(thresholds, dtype=np.float32) if thresholds is not None else None

    if "test_predictions" in results and "test_targets" in results:
        # k_fold mode from the provided XGBoost script.
        y_pred = np.asarray(results["test_predictions"]).astype(np.int32)
        y_true = np.asarray(results["test_targets"]).astype(np.int32)
        y_prob_raw = _first_existing(results, ["test_probabilities", "test_probs", "prob", "probs"])
    elif split == "train" and "train_pred" in results and "train_tgt" in results:
        y_pred = np.asarray(results["train_pred"]).astype(np.int32)
        y_true = np.asarray(results["train_tgt"]).astype(np.int32)
        y_prob_raw = _first_existing(results, ["train_probabilities", "train_probs", "train_prob"])
    elif split == "val" and "val_pred" in results and "val_tgt" in results:
        y_pred = np.asarray(results["val_pred"]).astype(np.int32)
        y_true = np.asarray(results["val_tgt"]).astype(np.int32)
        y_prob_raw = _first_existing(results, ["val_probabilities", "val_probs", "val_prob"])
    elif "pred" in results and "tgt" in results:
        # original mode test predictions from the provided XGBoost script.
        y_pred = np.asarray(results["pred"]).astype(np.int32)
        y_true = np.asarray(results["tgt"]).astype(np.int32)
        y_prob_raw = _first_existing(results, ["prob", "probs", "test_probabilities", "test_probs"])
    else:
        raise ValueError("Unsupported results.pickle format. Expected original or k_fold XGBoost keys.")

    y_prob = np.asarray(y_prob_raw, dtype=np.float32) if y_prob_raw is not None else None
    sample_indices = np.arange(y_true.shape[0], dtype=np.int64)
    return PredictionBundle(labels=y_true, probs=y_prob, preds=y_pred, sample_indices=sample_indices), thresholds_np, results
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Detailed error analysis for XGBoost MultiOutputClassifier functional-group models.")

    parser.add_argument("--output-dir", type=Path, default="benchmark/xgboost/results_whole/evaluation")

    # Fast path: use saved results.pickle directly.
    parser.add_argument("--results-pickle", type=Path, default=None)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="In full reload mode, use model.predict() labels instead of thresholded predict_proba().",
    )
...
def main() -> None:
    ...
    if args.results_pickle is not None:
        ...
        bundle, pickle_thresholds, results_metadata = load_bundle_from_results_pickle(args.results_pickle, split=args.split)
        if thresholds is None:
            thresholds = pickle_thresholds
        if bundle.probs is None:
            print("Warning: results.pickle has no probabilities. AP/AUC, threshold sensitivity, and probability plots are skipped.")
        ...
        print(f"Loading XGBoost model from {args.model_checkpoint}...")
        model = load_xgboost_model(args.model_checkpoint)

        print(f"Running probability inference on split={args.split} ({len(selected['X'])} samples)...")
        probs = predict_xgboost_probs(model, selected["X"])
        ...
        if args.retune_thresholds_on != "none":
            ...
            tune_probs = predict_xgboost_probs(model, tune_payload["X"])
        ...
        if args.use_model_predictions:
            print("Using model.predict() labels for evaluation.")
            preds = predict_xgboost_labels(model, selected["X"])
        else:
            print("Using thresholded predict_proba() labels for evaluation.")
