    ...
    with open(job_log, "w") as out:
        subprocess.call(
            [
                ...
                "1",
                "-R",
                "rusage[ngpus=1,mem=25,cpu=8]",
                "-o",
                str(Path(output_path) / "test.out"),
                "-e",
                str(Path(output_path) / "test.err"),
                "onmt_train",
                "-config",
                input_file_path,
            ],
...
parser = argparse.ArgumentParser(
    description="Run transformer training from data creation to inference."
)
parser.add_argument("--output_path", required=True, help="Output folder")
parser.add_argument(
    "--template_path", default="./benchmark/transformer_template.yaml", help="Output folder"
)
